...
> [!NOTE]
> npm/yarn workspaces are diffed member-by-member; pnpm's `pnpm-workspace.yaml` isn't read yet, so a pnpm monorepo only has its root manifest's own dependencies diffed for now — see the [roadmap](ROADMAP.md).
